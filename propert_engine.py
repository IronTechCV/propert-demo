# -*- coding: utf-8 -*-
"""properT-engine.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/10AdAydLhCHRUYN4WD7xutdpYjwVpY4wn
"""
...
def output(recognised_obj):
  Subs = regulations_df[regulations_df["item"] == recognised_obj]
  Subs.index = pd.RangeIndex(len(Subs.index))

  print('Item: ' + recognised_obj)

  # Display choices, ideally dropdown box
  for i in range(len(Subs["choice"])):
    print(str(i+1) + '. ' + Subs.loc[i, "choice"])

  # Get choice 
  c = input('Please input your choice: ')
  r = range(0, len(Subs["choice"]))

  assert int(c)-1 in r, "Wrong text"
  # Output regulation
  
  print('\nAdvice: ' + Subs.loc[int(c)-1, "text"])

  # report
  report(Subs.loc[int(c)-1, "target"], 
         recognised_obj, 
         Subs.loc[int(c)-1, "choice"], 
         Subs.loc[int(c)-1, "text"])
...
if b'Stairs' in all_objs:
  output("Stairs")
elif b'Window' in all_objs:
  output('Window')
elif b'Dog' in all_objs or b'Cat' in all_objs:
  output('Pet')
else: 
  print('Object not found in legal database')