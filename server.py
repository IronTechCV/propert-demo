...
def run_detector(detector, path):
  ...
  return export, [item.decode('ascii') for item in temp_objs]
...
def detect_img(image_url):
  ...
  output_img, items = run_detector(detector, image_path)
  end_time = time.time()
  return output_img, items

#@title Retrieve provision from legal database
@anvil.server.callable
def recognise_objs(all_objs):
  interested = []
  if 'Stairs' in all_objs:
    interested.append("Stairs")
  if 'Window' in all_objs:
    interested.append('Window')
  if 'Dog' in all_objs or 'Cat' in all_objs:
